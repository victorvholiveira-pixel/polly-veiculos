  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <Link
          to="/estoque/novo"
          className="rounded-lg bg-slate-900 px-3 py-1.5 text-sm font-medium text-white dark:bg-slate-50 dark:text-slate-900"
        >
          + Adicionar
        </Link>
      </div>
      ...
      {!loading && !error && filtered.length === 0 && (
        <p className="text-slate-500 dark:text-slate-400">
          {vehicles.length === 0 ? 'Nenhum veículo cadastrado ainda.' : 'Nada encontrado para essa busca.'}
        </p>
      )}
